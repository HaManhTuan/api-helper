...
from app.exceptions import ForbiddenException, InactiveUserException, TokenExpiredException, TokenInvalidException
...
async def get_current_user(token: str = Depends(oauth2_scheme), db: AsyncSession = Depends(get_db)) -> User:
    ...
    try:
        ...
        if user_data:
            ...
            identifier: Optional[str] = user_data.get("sub")
        ...
        user = await user_service.get_by_identifier(db, identifier=identifier)

        if user is None:
            logger.warning(f"User from token not found: {identifier}")
            raise credentials_exception
...
async def get_current_active_user(
    ...
    if not current_user.is_active:
        logger.warning(f"Inactive user attempt: {current_user.identifier}")
...
async def get_current_admin_user(current_user: User = Depends(get_current_user)) -> User:
    """
    Enforce admin-only access for protected admin routes.
    """
